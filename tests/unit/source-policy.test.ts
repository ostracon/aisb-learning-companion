...
describe("source policy", () => {
  it("allows only explicit visible curriculum and participant file classes", () => {
    ...
    expect(classifyRelativeAisbPath("3.1-tokenization/README.md")).toMatchObject({ allowed: true });
  ...
  it("rejects traversal and secret-looking files", () => {
    ...
    expect(classifyRelativeAisbPath("1.1-llm-internals/private-key/README.md")).toMatchObject({ allowed: false });
  });
});
